...
export const Player = () => {
    ...
    const logic = async () => {
        ...
        const filterRes = ["4k", "3d"]
        const lowestResStream = res.data.movie.torrents.filter(torrent => !filterRes.includes(torrent.quality.toLocaleLowerCase())).find(torrent => torrent.quality === (res.data.movie.torrents.map(torrent => parseInt(torrent.quality.slice(0, -1))).sort((a, b) => a - b)[0] + "p"));
        console.log(lowestResStream)
